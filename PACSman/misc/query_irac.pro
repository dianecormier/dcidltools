...
;
; PURPOSE:
;    Query the IRSA server and return the first Spitzer/IRAC image found for a
;    given object
;
; EXPLANATION:
...
;
; CALLING SEQUENCE:
;      QueryIRAC, object_name, Im, Hdr, BAND=
;
; INPUTS:
;      object_name - object name
;
;      band - IRAC band: 'ch1', 'ch2', 'ch3', 'ch4'
;
...
pro query_irac, object, image, header, band = band
  ...
  q = sm_webget("http://irsa.ipac.caltech.edu/cgi-bin/Spitzer/Spitzer/nph-spitzer?locstr="+object+"&iracim="+iracim+"&mode=prog")
  r = stregex(q.text, 'http.*iracim.*bat', /extract)
  ind = where(r ne '', c)
